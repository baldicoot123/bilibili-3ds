...
typedef struct {
	bool login, settings, hist, fav, popular, recommend, cache, cache_list;
} ListActions;
...
static void draw_bottom_list(bool touched, float tx, float ty, ListActions *act) {
	...
	if (ui_button(10, 28, 145, 36, "推荐",
	              hl == MODE_RECOMMEND ? UI_COL_ACCENT : UI_COL_SEL,
	              touched, tx, ty)) {
		act->recommend = true;
		s_hl_mode = MODE_RECOMMEND;
	}
	if (ui_button(165, 28, 145, 36, "热门",
	              hl == MODE_POPULAR ? UI_COL_ACCENT : UI_COL_SEL,
	              touched, tx, ty)) {
		act->popular = true;
		s_hl_mode = MODE_POPULAR;
	}
	if (ui_button(10, 70, 145, 36, "历史",
	              hl == MODE_HISTORY ? UI_COL_ACCENT : UI_COL_SEL,
	              touched, tx, ty)) {
		act->hist = true;
		s_hl_mode = MODE_HISTORY;
	}
	if (ui_button(165, 70, 145, 36, "收藏",
	              hl == MODE_FAV ? UI_COL_ACCENT : UI_COL_SEL,
	              touched, tx, ty)) {
		act->fav = true;
		s_hl_mode = MODE_FAV;
	}
	if (ui_button(10, 112, 145, 36, "设置", UI_COL_SEL, touched, tx, ty))
		act->settings = true;
	if (ui_button(165, 112, 145, 36, bili_logged_in() ? "注销" : "扫码登录",
	              UI_COL_SEL, touched, tx, ty))
		act->login = true;
	if (ui_button(10, 154, 145, 36, "缓存当前视频", UI_COL_ACCENT,
	              touched, tx, ty))
		act->cache = true;
	if (ui_button(165, 154, 145, 36, "缓存列表", UI_COL_SEL,
	              touched, tx, ty))
		act->cache_list = true;
}
...
static void cache_selected(void) {
	...
	int r = cache_manager_enqueue(v->bvid, v->cid, v->aid, v->title,
	                              v->author, g_qn);
	if (r == 0) {
		snprintf(s_busy, sizeof(s_busy), "已加入缓存队列");
		download_worker_wake();
	} else if (r == 1) {
		snprintf(s_busy, sizeof(s_busy), "这个视频已在缓存列表");
	} else if (r == -2) {
		snprintf(s_busy, sizeof(s_busy), "缓存队列已满(%d项)", CACHE_MAX_TASKS);
	} else if (r == -4) {
		snprintf(s_busy, sizeof(s_busy), "同名文件冲突，请检查SD卡缓存目录");
	} else {
		snprintf(s_busy, sizeof(s_busy), "加入缓存失败:%d", r);
	}
	if (!net_is_shutting_down()) thumb_start(s_list, s_count);
}
